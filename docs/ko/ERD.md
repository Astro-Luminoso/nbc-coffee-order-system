...
수집 플랫폼을 호출하기 전에 아직 `PENDING`인 주문 행을 DB 비관적 잠금으로 가져옵니다. 성공하면
`SUCCEEDED`로 바뀌고, 실패하면 `PENDING`으로 남아 재시도 스케줄러가 다음 실행에서 다시 시도합니다.
이 잠금은 정상적인 여러 인스턴스가 같은 주문을 동시에 호출하지 못하게 합니다. 다만 수집 플랫폼의
호출 성공과 DB 커밋은 원자적이지 않으므로 전달은 at-least-once 방식이며, 수집 플랫폼은 주문 ID로
중복을 제거해야 합니다.
...
- 전송 시도는 외부 수집 플랫폼을 호출하기 전에 아직 대기 상태인 주문 행을 비관적으로 잠급니다.
  전송 실패는 대기 상태로 남고 재시도는 주문 ID를 외부 멱등성 식별자로 사용합니다.
- 외부 호출 결과와 로컬 `SUCCEEDED` 커밋은 하나의 원자 작업이 아니므로 전달은 at-least-once
  방식이며 수신 측은 주문 ID로 중복을 제거합니다.
