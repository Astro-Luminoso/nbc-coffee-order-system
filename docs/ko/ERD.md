...
권장 인덱스는 7일 인기 메뉴 집계를 위한 `(ordered_at, menu_id)`입니다. 운영 규모에서 대기
주문 재시도 조회가 많으면 `collection_status` 인덱스를 추가할 수 있습니다.

주문은 결제와 같은 트랜잭션에서 `collection_status = PENDING`으로 생성됩니다. 전송 성공 시
`SUCCEEDED`로 바뀝니다. 전송 실패 시 `PENDING`으로 남고 재시도 스케줄러가 다음 실행에서
다시 시도합니다. 전달은 at-least-once 방식이므로 여러 인스턴스가 같은 대기 주문을 전송할 수
있으며, 수집 플랫폼은 주문 ID로 중복을 제거해야 합니다.
...
- 전송 실패는 대기 상태로 남습니다. 재시도는 주문 ID를 외부 멱등성 식별자로 사용합니다.
...
이벤트는 같은 주문을 두 번 증가시킬 수 없습니다. 인기 메뉴 API는 현재 날짜를 제외한 직전
완료 7일 키를 합산합니다. 키가 없거나 예상치 않게 만료됐거나 캐시 검증에 실패하면 MySQL
주문을 집계해 키를 재구축합니다.
