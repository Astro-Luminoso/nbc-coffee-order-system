...
/**
 * 포인트 충전 트랜잭션 서비스의 도메인 조정 책임을 검증한다.
 */
@ExtendWith(MockitoExtension.class)
class PointChargeTransactionServiceTest {
    ...
    /**
     * 각 테스트 전에 실제 JSON 변환기를 사용하는 서비스를 구성한다.
     */
    @BeforeEach
    void setUp() {
        pointChargeTransactionService = new PointChargeTransactionService(
                userService,
                idempotencyService,
                new ObjectMapper()
        );
    ...
    void chargeCompletesIdempotencyRecordWithTheOriginalCommonResponse() throws Exception {
        ...
        PointChargeResponse response = pointChargeTransactionService.charge(1L, "charge-1", 10_000L);
